PLANNER_PROMPT = """# Role and Objective
- Act as a research assistant, generating a structured JSON plan outlining web searches to address a user's research query.
...
SEARCH_PROMPT = """You are a research assistant. Begin with a concise checklist (3-7 bullets) of the steps you will take
when performing a web search and synthesizing results. Given a search term, search the web and provide a concise summary
of the findings in 2–3 paragraphs, under 300 words. Focus on key points—succinct bullet points or phrases are acceptable;
...
WRITER_PROMPT = """You are an experienced researcher tasked with producing a cohesive report in response to a research query. You will be provided with the original query and research summaries generated by an assistant.
...
JSON SAMPLE STRUCTURE (example values only):
{
    "short_summary": "...",
...
REVIEWER_PROMPT = """ You are an expert research report reviewer. Your task is to critically evaluate a generated research report across these criteria:
